  ...
  const setLabels = useMutation(
    (labelId) => {
      const newLabels = labels.includes(labelId)
        ? labels.filter((currentLable) => currentLable !== labelId)
        : [...labels, labelId];
    ...
    <div className="issue-options">
      <div>
        ...
        {labelsQuery.isLoading
          ? null
          : labels.map((label) => {
              const labelObject = labelsQuery.data?.find((queryLabel) => {
                return queryLabel.id === label;
              });
      ...
      <GoGear
        onClick={() =>
          !labelsQuery.isLoading && setMenuOpen((prevMenu) => !prevMenu)
        }
      />
